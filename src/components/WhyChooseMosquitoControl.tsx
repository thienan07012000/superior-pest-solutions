...
import { MapPin, ShieldCheck, Crosshair, Clock, Leaf } from 'lucide-react';

const benefits = [
  {
    title: "Local Experts",
    description: "We live here too, so we already know where mosquitoes like to hide in Northwest Arkansas yards. That means we show up knowing exactly what to look for.",
    icon: <MapPin className="w-6 h-6" />,
    number: "01",
  },
  {
    title: "Safe Treatments",
    description: "Your kids and pets can go right back outside once the treatment dries. We use products that are tough on mosquitoes but gentle on your family.",
    icon: <ShieldCheck className="w-6 h-6" />,
    number: "02",
  },
  {
    title: "Targeted Approach",
    description: "We go straight to where mosquitoes are actually breeding and resting. No guesswork, no spraying everything and hoping for the best.",
    icon: <Crosshair className="w-6 h-6" />,
    number: "03",
  },
  {
    title: "Long-Lasting Results",
    description: "One treatment keeps working for weeks, so you're not calling us back every few days. You'll actually want to sit outside again.",
    icon: <Clock className="w-6 h-6" />,
    number: "04",
  },
  {
    title: "Eco-Friendly Options",
    description: "We care about your yard and the environment around it. Our products get rid of mosquitoes without being harsh on the plants, soil, or wildlife nearby.",
    icon: <Leaf className="w-6 h-6" />,
    number: "05",
  }
];

export const WhyChooseMosquitoControl = () => {
  return (
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-12 relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground tracking-tight leading-tight">
            ...
            <span className="text-[var(--brand-red)] inline-block mt-2">Superior Pest Solutions</span>
          </h2>
        </motion.div>
