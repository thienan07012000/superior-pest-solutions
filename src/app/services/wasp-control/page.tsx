...
import { WaspInfoSection } from "@/components/WaspInfoSection";
import { WaspSignsSection } from "@/components/WaspSignsSection";
import { WhyChooseWaspControl } from "@/components/WhyChooseWaspControl";
import { ReferralBand } from "@/components/ReferralBand";
import { AreasServedWaspControl } from "@/components/AreasServedWaspControl";
import { FAQ } from "@/components/FAQ";
import ReviewCarousel from "@/components/ReviewCarousel";

export const metadata: Metadata = {
  title: "BEST Wasp Control Northwest Arkansas - if you're looking for Wasp Nest Removal, Wasp Extermination, or Wasp Prevention near me - Superior Pest Solutions is the place to call",
  description: "Superior Pest Solutions provides expert wasp control throughout Northwest Arkansas - locating nests and eliminating them safely with family-friendly treatments. Guaranteed results and a free quote waiting for you.",
  alternates: {
    canonical: '/services/wasp-control',
  },
};

const waspFaqs = [
  {
    question: "Is it safe to remove a wasp nest on my own?",
    answer: "We do not recommend it. Wasps can sting multiple times and will swarm when their nest is disturbed. Our technicians have the right equipment and products to remove nests safely without putting you or your family at risk."
  },
  {
    question: "Is the treatment safe for my kids and pets?",
    answer: "Yes, it is completely safe for everyone in your family once it dries. We use targeted products that only affect the wasps, so you can stop worrying."
  },
  {
    question: "How fast will the wasps go away?",
    answer: "Most nests are eliminated within 24 to 48 hours after treatment. You might still see a few wasps returning to where the nest was, but they will not be able to rebuild once we have treated the area."
  },
  {
    question: "Do I need to leave my house when you come to treat it?",
    answer: "Most of the time, you can stay right at home. Our treatments are applied directly to the nests and nesting areas. If we ever need you to step out for a bit, we will always let you know well ahead of time."
  },
  {
    question: "Why do wasps keep coming back to the same spot?",
    answer: "Wasps leave behind pheromones that attract new queens to the same location the following year. We treat the area to remove these chemical markers and recommend sealing entry points to prevent them from rebuilding."
  },
  {
    question: "Will you make me sign a long contract?",
    answer: "No, we will not. We do not like forcing people into contracts. We will fix your wasp problem and we can talk about prevention plans to keep them away, but you are never locked in."
  },
  {
    question: "What happens if wasps come back after you treat my home?",
    answer: "Just give us a call. We want to make sure your home stays wasp free. If they show up again before your next planned visit, we will come back out and take care of it for free."
  }
];
...
export default function WaspControlPage() {
    ...
    "description": "Professional wasp control in Northwest Arkansas by Superior Pest Solutions. Effective, safe, and guaranteed."
};
  ...
  return (
    <div className="flex flex-col w-full relative">
      ...
      */}
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10 py-12 lg:py-0">
          ...
          <div className="flex flex-col items-start max-w-2xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white shadow-sm border border-black/5 mb-8 transform transition hover:scale-105">
              ...
              <span className="text-secondary font-medium tracking-wide uppercase text-foreground/80">Wasp Control</span>
            </div>

            <h1 className="mb-6 text-foreground">
              Wasp Control in <span className="text-[var(--brand-red)]">Northwest Arkansas</span>
            </h1>

            <p className="text-lg md:text-xl text-foreground/80 mb-10 leading-relaxed max-w-xl">
              Whether it is paper wasps under your eaves or yellow jackets in the ground, we find every nest and get rid of it. No more dodging wasps on your own porch. Fast, safe, and done right.
            </p>
          ...
          <div className="relative w-full h-[400px] md:h-[500px] lg:h-[600px] rounded-[2rem] overflow-hidden shadow-[0_32px_80px_-16px_rgba(0,0,0,0.18)] group lg:translate-x-8">
            ...
            <Image
              src="/wasps.webp"
              alt="Professional pest control technician removing a wasp nest safely"
              fill
      ...
      <WaspInfoSection />
      <WaspSignsSection />
      <WhyChooseWaspControl />
      <ReferralBand
        title="Help a neighbor get rid of their wasp problem."
        description={<>Refer someone for wasp control and you both snag <span className="text-white font-bold underline underline-offset-4 decoration-white/30">50% OFF</span> your next visit!</>}
      />
      <AreasServedWaspControl />
      <FAQ
        faqs={waspFaqs}
        title={<>Answers to your <span className="text-[var(--brand-red)]">wasp</span> questions</>}
